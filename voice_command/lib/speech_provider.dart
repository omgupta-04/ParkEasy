...
class SpeechProvider extends ChangeNotifier {
  ...
  Future<void> _initWakeWord() async {
    ...
    const accessKey = "kT6e7AoFQGKX0SxpntSrTkAr0/kfpbw8yQuPwIhf8uH4CgnFPSIiDQ=="; // Replace with your real access key
    const keywordPath = 'assets/hey_park_easy.ppn';
  ...
  void _onStatus(String status) async {
    if (status == 'done') { // for fast response, you can do it at status == 'notListening' but not both else it will be called 2 times
      _isListening = false;
      _speechToText.stop();
      notifyListeners();
      await _porcupineManager?.start();
